...
-   **程序 ID**: 5y3FcQs2Ar6kTqpsJpJdHRJih3G9WDtsmkiNX54UtPiq
-   **版本**: 0.1.0
-   **Anchor 规范**: 0.1.0
...
#### create_product_atomic ⭐ **已优化**

原子化创建商品（优化版本，已移除 payer 参数）

**参数**:

-   `name: String` - 商品名称
-   `description: String` - 商品描述
-   `price: u64` - 商品价格（SOL，以 lamports 为单位）
-   `keywords: Vec<String>` - 关键词列表
-   `payment_token: Pubkey` - 支付代币地址
-   `token_decimals: u8` - 代币精度
-   `token_price: u64` - 代币价格

**账户**:

-   `merchant` (mut, signer) - 商户账户（同时作为支付者）
-   `global_root` (mut) - 全局根账户
-   `merchant_id_account` (mut) - 商户 ID 账户
-   `merchant_info` (mut) - 商户信息账户
...
-   `product` - 商品账户
-   `escrow_account` (mut) - 托管账户
-   `order` (mut) - 订单账户
-   `order_stats` (mut) - 订单统计账户
...
#### initialize_keyword_index_if_needed
...
-   `keyword_root` (mut) - 关键词根账户
-   `first_shard` (mut) - 第一个分片
-   `payer` (mut, signer) - 支付账户
-   `system_program` - 系统程序

#### add_product_to_keyword_index_if_needed
...
#### initialize_price_index_if_needed
...
-   `payer` (mut, signer) - 支付账户
-   `price_index` (mut) - 价格索引账户
-   `system_program` - 系统程序

#### add_product_to_price_index_if_needed
...
#### initialize_sales_index_if_needed
...
-   `payer` (mut, signer) - 支付账户
-   `sales_index` (mut) - 销量索引账户
-   `system_program` - 系统程序

#### add_product_to_sales_index_if_needed
...
所有优化都保持了功能的完整性，现有的业务逻辑和数据结构保持不变，只是简化了指令的账户要求和调用方式。
